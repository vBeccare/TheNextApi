...
public class CarrinhoController {
	...
	@GetMapping("/itens")
	public ResponseEntity<List<ItensCompra>> itens() {
		List<ItensCompra> result = itensRepository.findAll();
		return ResponseEntity.ok(result);
	}
	...
	public ResponseEntity<CarrinhoCompra> atualizarStatus(@Valid @RequestBody CarrinhoCompra carrinhoCompra) {
		return carrinhoService.atualizarStatus(carrinhoCompra)
				.map(resposta -> ResponseEntity.status(HttpStatus.CREATED).body(resposta))
				.orElse(ResponseEntity.status(HttpStatus.BAD_REQUEST).build());
	}
